...
.PHONY: fmt
fmt: ## Run go fmt and goimports against code.
	go fmt ./...
	@command -v goimports >/dev/null 2>&1 || { echo "Installing goimports..."; go install golang.org/x/tools/cmd/goimports@latest; }
	@PATH="$(PATH):$(shell go env GOPATH)/bin" goimports -w .
...
KIND_CLUSTER ?= previewd-test-e2e
...
.PHONY: cover
cover: manifests generate fmt vet setup-envtest ## Run tests with coverage report
...
.PHONY: bench
bench: manifests generate setup-envtest ## Run benchmark tests
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" \
		go test -bench=. -benchmem $$(go list ./... | grep -v /e2e)
...
.PHONY: quality-full
quality-full: quality test-race cover ## Run comprehensive quality checks (includes race detection and coverage)
...
.PHONY: verify
verify: quality ## Verify code is ready for commit
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name previewd-builder
	$(CONTAINER_TOOL) buildx use previewd-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm previewd-builder
	rm Dockerfile.cross
